...
async function main() {
  ...
  for (const factory of Factories) {
    ...
    const data = hre.ethers.utils.hexlify(
      hre.ethers.utils.concat([
        Factory.bytecode, //
        hre.ethers.utils.defaultAbiCoder.encode(['address'], ['0x5C1e26EAA60b042a304822e6449aA8Ef1Ac6aEFE']),
      ])
    )
    const txn = await (
  ...
  await Promise.all(
    map(resultsAddr, (addr, idx) => {
      return hre.run('verify:verify', {
        address: addr,
        constructorArguments: ['0x5C1e26EAA60b042a304822e6449aA8Ef1Ac6aEFE'],
      })
    })
  )
}
